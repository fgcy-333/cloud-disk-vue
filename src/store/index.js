...
export default new Vuex.Store({
    state: {
        ...
        //是否已经从localStorage中获取了用户第一层的文件夹id
        hasCurrentFolderId: false,
    ...
    mutations: {
        ...
        //返回根目录
        gotoRoot(state) {
            state.historyStackPoint = 0;
            let root = state.historyStack[0];
            state.historyStack = []
            state.historyStack.push(root)
        },
        //更新历史栈信息
        updateHistoryStackInfo(state, obj) {
            state.historyStack.push(obj);
            state.historyStackPoint++;
        },
        //更新是否拥有第一层文件夹的id
        updateHasCurrentFolderId(state, flag) {
            state.hasCurrentFolderId = flag;
        },

        updateCurrentFolderId(state, folderId) {
            state.currentFolderId = folderId;
        },
        backHistoryStack(state, index) {
            if (index < 0 || index > state.historyStackPoint) {
                console.log("版本回退出现问题,传入的下标有问题:", index)
            }
            //删除元素
            for (let i = 1; i <= state.historyStackPoint - index; i++) {
                state.historyStack.pop();
            }
            //修改历史栈下标
            state.historyStackPoint = index;
        },
